...
**Note:** `root` is a reserved keyword — it refers to `.md` files directly in `.hal/standards/` (not in a subfolder).
...
Read `.hal/standards/index.yml` to get the list of available standards and their descriptions.
...
@.hal/standards/api/response-format.md
@.hal/standards/api/error-handling.md
@.hal/standards/global/naming.md
...
Arguments can be:
- **Folder name** — `api` → inject all `.md` files in `.hal/standards/api/`
- **Folder/file** — `api/response-format` → inject `.hal/standards/api/response-format.md`
- **Root folder** — `root` → inject all `.md` files directly in `.hal/standards/` (not in subfolders)
- **Root file** — `root/naming` → inject `.hal/standards/naming.md`
