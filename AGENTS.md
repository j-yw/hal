...
- Success-path worker lifecycle scenarios can stay deterministic without a live worker by seeding harness state in order (`TransitionRun` queued→claimed→running→succeeded, `CreateAttempt` + terminal `TransitionAttempt`, then `UpdateRunSnapshotRefs`) and asserting through harness query helpers (`RunTransitions`, `AttemptTerminalizations`, `SnapshotRefs`).
