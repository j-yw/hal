...
- Worker lifecycle scenarios should source status/logs/pull required-key contracts from `workerLifecycleJSONContractFixtures` and decode through `mustDecodeWorkerLifecycleJSONOutput` so adapter/workflow matrices share one camelCase JSON assertion surface.
- In worker lifecycle scenarios, assert workflow submit payloads with `assertWorkerLifecycleCanonicalJSONContract` (required camelCase keys + forbidden snake_case aliases), and assert status/logs/pull payloads via `assertWorkerLifecycleCheckpointJSONContract` so contract checks stay centralized.
