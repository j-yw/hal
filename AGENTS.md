...
- Product context (`.hal/product/*`) is durable, not feature state: do not include `template.ProductDir`/product docs in archive state lists, and `archive.Restore` must skip archived `product/` entries so current product docs are never overwritten.
