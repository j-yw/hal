...
- Lifecycle logs scenarios should seed events through the harness store (`h.Store.InsertEvent`) after submission because `SubmitWithBundle` persists runs/snapshots but does not emit timeline events on its own.
