...
- Phase 25 network proxy bridge helpers belong in `internal/sandbox/credential_proxy_network_proxy.go`; keep them data-only, copy only `SandboxNetworkProxySessionMetadata.ID` and safe policy snapshot identity into sanitized credential proxy plan/session metadata, and keep request/destination handling limited to Phase 24 safe categories without raw hosts, URLs, ports, headers, bodies, or enforcement metadata.
...
- Phase 25 verification docs live in `docs/design/sandbox-runtime-v2-phase25-credential-proxy-plan-verification.md` and are guarded by `cmd/phase25_credential_proxy_docs_test.go`; keep focused commands, fake-only scope, unchanged command/manifest/factory/timeline JSON surfaces, and future-phase live-delivery handoffs in sync with credential proxy metadata behavior.
- Phase 25 fake-only verification coverage lives in `cmd/phase25_credential_proxy_docs_test.go`; assert documented focused `go test` selectors still match required credential proxy tests, and scan default Phase 25 test files for integration build tags, live environment requirements, and live dependency imports using parsed imports rather than broad raw-source marker scans that would trip seeded fixture strings.
...
- Preserve unrelated project configuration sections when updating project-scoped sandbox settings; use structured YAML round-tripping rather than replacing `.hal/config.yaml` wholesale.
...
- `internal/sandbox/migrate.go` config migration should treat global config as authoritative: if `sandbox-config.yaml` already exists, skip local migration; otherwise copy only supported local `.hal/config.yaml` sandbox settings, preserving the local file unchanged.
