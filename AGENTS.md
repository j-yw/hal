...
- Credential proxy validation belongs in `internal/sandbox/credential_proxy_validation.go`; keep it pure and data-only, validate safe IDs/references and explicit enum allowlists, reject raw-looking values with sanitized code/field/message errors only, and leave normalization, durable sanitizers, manifest/factory plumbing, and live behavior to later phases.
