...
- Verification config also lives in `internal/verify`: `LoadConfig(dir)` reads `.hal/config.yaml` `verify.checks`, defaults omitted `required` to true, defaults omitted/empty `workDir` to the absolute project root, and reports validation errors as `verify.checks[n].field ...`.
- Verification shell execution lives in `internal/verify.Run`: it runs each `ShellCheck` via the configured command, workDir, and timeout, then aggregates check results into the verify-v1 summary/status contract.
- Optional verification check problems keep their concrete check status (`fail`, `timeout`, or `missing`) and add top-level `Warning` entries; only required check problems should drive top-level `status=fail`.
- Verify artifact capture writes non-empty stdout/stderr under `.hal/reports/verify/<check-id>-stdout.txt` and `<check-id>-stderr.txt`; `LoadConfig` sets `Config.ProjectRoot` so `Run` can write artifacts relative to the project root while emitting relative paths in JSON.
- Factory run record verification metadata reuses `internal/verify` types (`verify.Summary` and `verify.ArtifactReference`) under `factory.VerificationRecord`; when changing it, keep `docs/contracts/factory-status-v1.md`, `docs/contracts/examples/factory-status-v1.json`, and `cmd/contracts_doc_test.go` synchronized.
- Verify shell timeouts use `internal/verify/sysproc_unix.go` and `sysproc_windows.go`; wire both `cmd.SysProcAttr = newSysProcAttr()` and `setupProcessCleanup(cmd)` for shell executions so Unix timeouts kill the whole process group.
- Verify execution tests use `helperCommand`/`TestVerifyHelperProcess` in `internal/verify/run_test.go` to invoke the current test binary through the shell; extend helper subcommands for deterministic pass/fail/timeout/artifact scenarios instead of relying on host-specific utilities.
- `hal verify --json` should emit the verify-v1 result to stdout before returning `ExitCodeExpectedNonZero` with a nil wrapped error for `status=fail`; this preserves parseable JSON stdout without extra error text while still producing a non-zero process exit.
