...
- Factory run result payloads should be built from the saved `factory.RunRecord` and `Store.LoadEvents` after terminal status is persisted, so `hal factory run --json` reflects durable state rather than transient in-memory state.
