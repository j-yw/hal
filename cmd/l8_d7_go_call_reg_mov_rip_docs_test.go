...
const l8D7GoCallRegMOVRIPDoc = "sandbox-runtime-v2-l8-d7-go-call-reg-mov-rip.md"

func TestL8D7GoCallRegMOVRIPVerificationDocument(t *testing.T) {
	doc := strings.Join(strings.Fields(readL8CredentialDeliveryFile(t, filepath.Join("..", "docs", "design", l8D7GoCallRegMOVRIPDoc))), " ")
	for _, required := range []string{
		...
		"unique non-writable ELF range",
		"Jump-table entries that skip the MOV",
		"traversed subset",
		...
		if !strings.Contains(doc, required) {
			t.Fatalf("Go CALL-reg MOV RIP verification document omits %q", required)
		}
	}
	for _, forbidden := range []string{
		...
		if strings.Contains(doc, forbidden) {
			t.Fatalf("Go CALL-reg MOV RIP verification document contains forbidden claim %q", forbidden)
		}
	}
}
