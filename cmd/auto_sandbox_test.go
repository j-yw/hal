...
func TestBuildAutoSandboxRemoteCommandPreservesAutoFlags(t *testing.T) {
	...
	for _, disallowed := range []string{"--sandbox", "--sandbox-name", "auto-box", "--sandbox-host", "worker-1", "--sandbox-runtime", sandboxruntime.DriverRootlessPodman, "--" + sandboxSyncOutFlagName, "--" + sandboxApplyFlagName} {
...
func TestCollectAutoSandboxGeneratedArtifactsKeepsRecoveryGenerationBestEffort(t *testing.T) {
	store := sandboxexecution.NewStore(filepath.Join(t.TempDir(), "sandbox-executions"))
	if err := store.SaveManifest(&sandboxexecution.Manifest{
		ID:        "auto-recovery-best-effort",
		Purpose:   sandboxexecution.PurposeAuto,
		Status:    sandboxexecution.StatusRunning,
		StartedAt: time.Date(2026, 7, 7, 12, 35, 0, 0, time.UTC),
	}); err != nil {
	...
	err := collectAutoSandboxGeneratedArtifacts(context.Background(), store, autoSandboxRequest{
		ExecutionID: "auto-recovery-best-effort",
		WorkDir:     workspace,
	}, autoSandboxExecutionResult{
		RuntimeDriver: driver,
		Result: &sandboxexec.Result{Target: sandboxruntime.Target{
			Name: "auto-recovery-best-effort-box",
			Runtime: sandboxruntime.RuntimeState{
	...
	if err != nil {
		t.Fatalf("collectAutoSandboxGeneratedArtifacts() error = %v, want best-effort recovery warning", err)
	}
	if recoveryCopyOut {
		t.Fatal("recovery patch copy-out should not run after recovery generation failure")
	}
	manifest, err := store.LoadManifest("auto-recovery-best-effort")
	if err != nil {
		t.Fatalf("LoadManifest() error: %v", err)
	}
