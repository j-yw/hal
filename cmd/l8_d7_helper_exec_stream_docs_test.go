...
const l8D7HelperExecStreamVerificationDoc = "sandbox-runtime-v2-l8-d7-helper-exec-stream.md"

func TestL8D7HelperExecStreamVerificationContract(t *testing.T) {
	doc := strings.Join(strings.Fields(readL8CredentialDeliveryFile(t, filepath.Join("..", "docs", "design", l8D7HelperExecStreamVerificationDoc))), " ")
	for _, required := range []string{
		"default-off",
		"helper exec-stream payload send",
		...
		"helperExecStreamCanonicalPrefixBytes",
		"already-admitted controller stream records",
		...
		if !strings.Contains(doc, required) {
			t.Fatalf("D7 helper exec-stream verification document omits %q", required)
		}
	}
	for _, forbidden := range []string{
		...
		if strings.Contains(doc, forbidden) {
			t.Fatalf("D7 helper exec-stream verification document contains forbidden claim %q", forbidden)
		}
	}
}

func TestL8D7HelperExecStreamRemainsDefaultOff(t *testing.T) {
	root := filepath.Clean("..")
	...
	for _, path := range targets {
		...
		for _, marker := range []string{
			...
			if strings.Contains(source, marker) {
				t.Fatalf("default production path %s wires helper exec-stream %s", filepath.ToSlash(path), marker)
			}
		}
	}
}

func TestL8D7HelperExecStreamControlContractStillForbidsListenDial(t *testing.T) {
