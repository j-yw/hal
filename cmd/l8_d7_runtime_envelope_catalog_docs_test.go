...
const l8D7RuntimeEnvelopeCatalogDoc = "sandbox-runtime-v2-l8-d7-runtime-envelope-catalog.md"

func TestL8D7RuntimeEnvelopeCatalogVerificationDocument(t *testing.T) {
	doc := strings.Join(strings.Fields(readL8CredentialDeliveryFile(t, filepath.Join("..", "docs", "design", l8D7RuntimeEnvelopeCatalogDoc))), " ")
	for _, required := range []string{
		"HL8E remains unissued",
		"explicit D7 catalog/runtime-row authority",
		...
		"futex",
		"mmap",
		"rawSyscallNoError.abi0",
		"rawVforkSyscall.abi0",
		"Native `_start` extras",
		...
		if !strings.Contains(doc, required) {
			t.Fatalf("runtime-envelope catalog verification document omits %q", required)
		}
	}
	for _, forbidden := range []string{
		...
		if strings.Contains(doc, forbidden) {
			t.Fatalf("runtime-envelope catalog verification document contains forbidden claim %q", forbidden)
		}
	}
}
