...
func TestSandboxAuthRemoteInstallScriptRejectsNestedProfileSymlink(t *testing.T) {
	clearSandboxAuthCodexHome(t)
	localHome := t.TempDir()
	writeSandboxAuthTestFile(t, localHome, ".pi/agent/auth.json", "pi-auth")
	...
	remoteHome := t.TempDir()
	redirectedDir := t.TempDir()
	if err := os.MkdirAll(filepath.Join(remoteHome, ".pi"), 0o700); err != nil {
		t.Fatalf("MkdirAll(.pi) error: %v", err)
	}
	if err := os.Symlink(redirectedDir, filepath.Join(remoteHome, ".pi", "agent")); err != nil {
		t.Skipf("symlink unavailable: %v", err)
	}
	...
	output, err := cmd.CombinedOutput()
	if err == nil {
		t.Fatalf("remote install script succeeded with nested symlinked target; output:\n%s", output)
	}
	if !strings.Contains(string(output), "refusing to install auth into symlinked target: .pi/agent") {
		t.Fatalf("remote install script output = %q", output)
	}
	if _, err := os.Stat(filepath.Join(redirectedDir, "auth.json")); !os.IsNotExist(err) {
		t.Fatalf("redirected auth file stat error = %v, want not exist", err)
