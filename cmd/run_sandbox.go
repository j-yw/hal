...
func runRunSandboxWithWriter(ctx context.Context, cmd *cobra.Command, args []string, opts runSandboxOptions, out, errOut io.Writer, deps runSandboxDeps) error {
	...
	execResult, execErr := deps.execute(ctx, req, commandOut, errOut, runSandboxExecutionHooks{
		OnTargetReady: func(ready *sandbox.SandboxState) error {
			...
			if err := saveRunSandboxManifest(store, req, sandboxexecution.StatusRunning, startedAt, nil, target); err != nil {
				return err
			}
	...
	result, err := sandboxexec.Run(ctx, sandboxexec.CommandRequest{
		...
		PrepareWorkspace: func(ctx context.Context, prep sandboxexec.PrepareContext, _ *sandboxexec.CommandRequest) error {
			if isGitBundleWorkspace(req.Workspace) {
				...
				_, err = deps.prepareCommandContext(ctx, prep, req.ProjectDir, req.WorkDir, prepOut)
				return err
			}
			if runSandboxWorkerRuntimeRouteSelected(req, prep.Target, selectedTarget) {
				if err := deps.bootstrapRunSandboxWorkspaceRuntime(ctx, req, prep, prepOut); err != nil {
					return err
				}
				if deps.prepareCommandContext == nil {
					return nil
			...
			if err := deps.bootstrapRunSandboxWorkspace(ctx, req, provider, prep, prepOut); err != nil {
				return err
			}
			if deps.prepareCommandContext == nil {
				return nil
...
func collectRunSandboxGeneratedArtifacts(ctx context.Context, store sandboxexecution.Store, req runSandboxRequest, result runSandboxExecutionResult) error {
	...
	if req.SyncOut.Enabled {
		...
		if _, err := sandboxexecution.CollectCommittedSyncOutArtifactBestEffort(ctx, sandboxexecution.CommittedSyncOutCollectionRequest{
			...
			return fmt.Errorf("collect run sandbox committed sync-out artifact: %w", err)
		}
	}
...
func applyRunSandboxSecurityReadinessGateError(req *runSandboxRequest, err error) {
	if req == nil {
		return
	}
...
type sandboxExecutionSecurityReadinessGateBlockedError struct {
...
func enforceSandboxExecutionSecurityReadinessGate(store sandboxexecution.Store, executionID string) error {
	...
	if err != nil || manifest == nil || manifest.Security == nil || manifest.Security.SecurityReadinessGate == nil {
		return err
	}
	decision := sandbox.CloneSandboxSecurityCapabilityReadinessGateDecisionPtr(manifest.Security.SecurityReadinessGate)
	if decision == nil ||
		decision.PolicyMode != sandbox.SandboxSecurityCapabilityReadinessGatePolicyModeStrict ||
		decision.Outcome != sandbox.SandboxSecurityCapabilityReadinessGateOutcomeBlocked {
		return nil
	}
	return sandboxExecutionSecurityReadinessGateBlockedError{decision: *decision}
}
