...
const l8D7HelperPrepareFileVerificationDoc = "sandbox-runtime-v2-l8-d7-helper-prepare-file.md"

func TestL8D7HelperPrepareFileVerificationContract(t *testing.T) {
	doc := strings.Join(strings.Fields(readL8CredentialDeliveryFile(t, filepath.Join("..", "docs", "design", l8D7HelperPrepareFileVerificationDoc))), " ")
	for _, required := range []string{
		"default-off",
		"helper prepare-file payload send",
		...
		"helperPrepareFileOwner",
		"HelperPrepareFileBody",
		"already-admitted live prepare payload",
		"ErrClientControlDependencyUnaccepted",
		"MaxHelperFileAggregateBytes",
		"ClientPolicy",
		"fail-closed",
		...
		if !strings.Contains(doc, required) {
			t.Fatalf("D7 helper prepare-file verification document omits %q", required)
		}
	}
	for _, forbidden := range []string{
		...
		if strings.Contains(doc, forbidden) {
			t.Fatalf("D7 helper prepare-file verification document contains forbidden claim %q", forbidden)
		}
	}
}

func TestL8D7HelperPrepareFileRemainsDefaultOff(t *testing.T) {
	root := filepath.Clean("..")
	...
	err := filepath.WalkDir(root, func(path string, entry fs.DirEntry, walkErr error) error {
		...
		ast.Inspect(file, func(node ast.Node) bool {
			...
			case "HelperConnectionOwner", "VerifiedHelperStream", "newPreopenedHelperConnectionOwner",
				"newHelperPrepareFileSendPacket", "helperPrepareFileOwner", "dispatchHelperPrepareFiles",
				"copyControllerPrepareFileOwner":
				t.Errorf("production file %s references helper prepare-file %s", normalized, ident.Name)
			}
			return true
		})
		return nil
	...
	for _, path := range targets {
		...
		for _, marker := range []string{
			...
			"VerifiedHelperStream",
			"helperPrepareFileOwner",
			"newHelperPrepareFileSendPacket",
			"dispatchHelperPrepareFiles",
			"EncodeHelperPrepareFileBody",
			"copyControllerPrepareFileOwner",
		} {
			if strings.Contains(source, marker) {
				t.Fatalf("default production path %s wires helper prepare-file %s", filepath.ToSlash(path), marker)
			}
		}
	}
}

func TestL8D7HelperPrepareFileControlContractStillForbidsListenDial(t *testing.T) {
