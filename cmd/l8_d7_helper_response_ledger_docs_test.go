...
const l8D7HelperResponseLedgerVerificationDoc = "sandbox-runtime-v2-l8-d7-helper-response-ledger.md"

func TestL8D7HelperResponseLedgerVerificationContract(t *testing.T) {
	doc := strings.Join(strings.Fields(readL8CredentialDeliveryFile(t, filepath.Join("..", "docs", "design", l8D7HelperResponseLedgerVerificationDoc))), " ")
	for _, required := range []string{
		"default-off",
		"helper-response path",
		"exact active ledger",
		"HelperReceiveRequest",
		"HelperResponseBody",
		"requestType=prepare_commit",
		...
		if !strings.Contains(doc, required) {
			t.Fatalf("D7 helper response ledger verification document omits %q", required)
		}
	}
}

func TestL8D7HelperResponseLedgerRemainsDefaultOff(t *testing.T) {
	root := filepath.Clean("..")
	...
	err := filepath.WalkDir(root, func(path string, entry fs.DirEntry, walkErr error) error {
		...
		ast.Inspect(file, func(node ast.Node) bool {
			...
			case "HelperConnectionOwner", "VerifiedHelperStream", "newPreopenedHelperConnectionOwner",
				"helperActiveLedger", "mapHelperPrepareSuccessToV2", "mapHelperRenewSuccessToV2",
				"mapHelperRevokeSuccessToV2", "mapHelperExecSuccessToV2", "readHelperResponsePacket":
				t.Errorf("production file %s references helper response ledger %s", normalized, ident.Name)
			}
			return true
		})
		return nil
	...
	for _, path := range targets {
		...
		for _, marker := range []string{
			...
			"mapHelperRevokeSuccessToV2",
			"mapHelperExecSuccessToV2",
			"readHelperResponsePacket",
			"ReceiveHelper",
		} {
			if strings.Contains(source, marker) {
				t.Fatalf("default production path %s wires helper response ledger %s", filepath.ToSlash(path), marker)
			}
		}
	}
}

func TestL8D7HelperResponseLedgerControlContractStillForbidsListenDial(t *testing.T) {
