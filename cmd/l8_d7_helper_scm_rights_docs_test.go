...
const l8D7HelperSCMRightsVerificationDoc = "sandbox-runtime-v2-l8-d7-helper-scm-rights.md"

func TestL8D7HelperSCMRightsVerificationContract(t *testing.T) {
	doc := strings.Join(strings.Fields(readL8CredentialDeliveryFile(t, filepath.Join("..", "docs", "design", l8D7HelperSCMRightsVerificationDoc))), " ")
	for _, required := range []string{
		...
		"newHelperSSHAcceptedPacket",
		"DecodeHelperSSHAcceptedFDBody",
		"recvmsg",
		"Exactly one SCM_RIGHTS descriptor",
		"consume-once",
		"never logged",
		"Destroy/cleanup errors override dispatch",
		"ExtensionSession.Handle",
		"commitExtensionPacketOwnership",
		...
		"does not mint JobCredential",
		"nil `HelperConnectionOwner`",
		"JobCredential active/cleanup proof minting stays unaccepted",
		...
		if !strings.Contains(doc, required) {
			t.Fatalf("D7 helper SCM_RIGHTS verification document omits %q", required)
		}
	}
	for _, forbidden := range []string{
		...
		if strings.Contains(doc, forbidden) {
			t.Fatalf("D7 helper SCM_RIGHTS verification document contains forbidden claim %q", forbidden)
		}
	}
}

func TestL8D7HelperSCMRightsRemainsDefaultOff(t *testing.T) {
	root := filepath.Clean("..")
	...
	err := filepath.WalkDir(root, func(path string, entry fs.DirEntry, walkErr error) error {
		...
		ast.Inspect(file, func(node ast.Node) bool {
			...
			case "HelperConnectionOwner", "VerifiedHelperStream", "newPreopenedHelperConnectionOwner",
				"tryReadHelperSSHAcceptedPacket", "drainIdleHelperSSHAccepted", "dispatchHelperSSHAccepted",
				"linuxSSHAcceptedConn", "newLinuxSSHAcceptedConn":
				t.Errorf("production file %s references helper SCM_RIGHTS %s", normalized, ident.Name)
			}
			return true
		})
		return nil
	...
	for _, path := range targets {
		...
		for _, marker := range []string{
			"HelperConnectionOwner",
			"VerifiedHelperStream",
			"tryReadHelperSSHAcceptedPacket",
			"drainIdleHelperSSHAccepted",
			"dispatchHelperSSHAccepted",
			"linuxSSHAcceptedConn",
			"DecodeHelperSSHAcceptedFDBody",
			"newHelperSSHAcceptedPacket",
		} {
			if strings.Contains(source, marker) {
				t.Fatalf("default production path %s wires helper SCM_RIGHTS %s", filepath.ToSlash(path), marker)
			}
		}
	}
}

func TestL8D7HelperSCMRightsControlContractStillForbidsListenDial(t *testing.T) {
