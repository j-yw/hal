...
func TestPhase21VerificationDocsCurrent(t *testing.T) {
	...
	required := []string{
		...
		"Production sync-out also generates an untracked tar archive and quoted file list from `git ls-files --others --exclude-standard`.",
		"Ignored files and Hal's generated sync-out payloads are excluded, empty untracked output is omitted",
		"both artifacts remain handoff-only rather than eligible for automatic host apply.",
