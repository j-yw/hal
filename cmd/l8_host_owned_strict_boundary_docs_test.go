...
func TestL8HostOwnedStrictBoundaryDocumentation(t *testing.T) {
	...
	for _, required := range []string{
		...
		"The private plan must not be passed to the current `ProcessLifecycleManager.StartProcess`",
		"The private command plan has no durable/public JSON shape",
		"consume the command and host path authority atomically",
		"replace the current `NamespaceProcessRunner` contract",
		"canonicalize and pin the Jailer and Firecracker executables",
		"require trusted non-unprivileged-writable parent paths",
		"verify a matching released/static binary pair",
		"validate dedicated UID/GID ownership",
		"set bounded Jailer resource/cgroup controls",
