...
func sandboxAuthRemoteInstallScript() string {
	return strings.Join([]string{
		...
		"mkdir -p \"$HOME\"",
		"tmp_dir=\"$(mktemp -d \"${TMPDIR:-/tmp}/hal-auth-sync.XXXXXX\")\"",
		"chmod 700 \"$tmp_dir\"",
		"cleanup_auth_tmp() {",
		"  rm -rf \"$tmp_dir\"",
		"}",
		"trap cleanup_auth_tmp EXIT HUP INT TERM",
		"tar -C \"$tmp_dir\" -xzf -",
		"ensure_auth_parent_dirs() {",
		"  rel=\"$1\"",
		"  dir_rel=\"${rel%/*}\"",
		"  if [ \"$dir_rel\" = \"$rel\" ]; then return 0; fi",
		"  current=\"$HOME\"",
		"  display=\"\"",
		"  old_ifs=\"$IFS\"",
		"  IFS=/",
		...
		"      echo \"refusing unsafe auth path: $rel\" >&2",
		"      exit 1",
		"    fi",
		"    current=\"$current/$part\"",
		"    if [ -z \"$display\" ]; then display=\"$part\"; else display=\"$display/$part\"; fi",
		"    if [ -L \"$current\" ]; then",
		"      echo \"refusing to install auth into symlinked target: $display\" >&2",
		"      exit 1",
		"    fi",
		"    if [ -e \"$current\" ] && [ ! -d \"$current\" ]; then",
		"      echo \"refusing to install auth into non-directory target: $display\" >&2",
		"      exit 1",
		"    fi",
		"    if [ ! -e \"$current\" ]; then",
		"      mkdir \"$current\"",
		...
		"      echo \"refusing to install auth into invalid target: $display\" >&2",
		"      exit 1",
		"    fi",
		"    chmod go-rwx \"$current\" 2>/dev/null || true",
		"  done",
		"}",
		"install_auth_file() {",
		"  rel=\"$1\"",
		"  src=\"$tmp_dir/$rel\"",
		"  if [ -L \"$src\" ]; then",
		"    echo \"refusing unsupported auth archive entry: $rel\" >&2",
		"    exit 1",
		"  fi",
		...
		"    exit 1",
		"  fi",
		"  ensure_auth_parent_dirs \"$rel\"",
		"  dest=\"$HOME/$rel\"",
		"  parent=\"${dest%/*}\"",
		"  base=\"${rel##*/}\"",
		"  if [ -L \"$dest\" ]; then",
		"    echo \"refusing to install auth into symlinked target: $rel\" >&2",
		"    exit 1",
		"  fi",
		"  if [ -e \"$dest\" ] && [ ! -f \"$dest\" ]; then",
		"    echo \"refusing to install auth into non-file target: $rel\" >&2",
		"    exit 1",
		"  fi",
		"  tmp_dest=\"$parent/.${base}.hal-auth-sync.$$\"",
		"  rm -f \"$tmp_dest\"",
		"  cp \"$src\" \"$tmp_dest\"",
		"  chmod go-rwx \"$tmp_dest\"",
		"  mv -f \"$tmp_dest\" \"$dest\"",
		"}",
		"for rel in .codex/auth.json .codex/version.json .codex/installation_id .pi/agent/auth.json .pi/agent/settings.json .pi/agent/trust.json .claude.json .claude/settings.json .claude/credentials.json .claude/.credentials.json; do",
		"  install_auth_file \"$rel\"",
		"done",
	}, "\n")
}
