...
func TestPhase35FirecrackerHostAdapterVerificationDocs(t *testing.T) {
	...
	required := []string{
		...
		"Phase 35 provides host-side implementations for `ProcessStarter`, `BootAcceptanceWaiter`, and `LiveProcessManager`.",
		"A future live backend path composes the starter into `BackendOptions.ProcessAdapter` with `firecracker.ProcessLaunchAdapter{Starter: adapter}`.",
		...
		"removes only state paths that match the tracked start-time Firecracker path plan for the live process handle",
