...
func TestDefaultCloudSubmitConfig(t *testing.T) {
	cfg := defaultCloudSubmitConfig()
	if cfg.IDFunc == nil {
		t.Fatal("defaultCloudSubmitConfig must provide IDFunc")
	}
	id := cfg.IDFunc()
	if id == "" {
		t.Fatal("defaultCloudSubmitConfig IDFunc returned empty ID")
	}
	if !strings.HasPrefix(id, "run-") {
		t.Fatalf("submit ID = %q, want run- prefix", id)
	}
}
...
func TestWriteJSON(t *testing.T) {
	...
	err := writeJSON(&buf, cloudErrorResponse{
		Error:     "test error",
		ErrorCode: "test_code",
	})
	if err != nil {
