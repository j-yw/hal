...
func TestL5ArchitectureContainsMandatoryPhaseSections(t *testing.T) {
	...
	for _, required := range []string{
		...
		"exclusive per-runtime lifecycle reservation",
		"cannot race with an in-flight start that later becomes ready",
		"Host-process ownership is recorded immediately after launch",
		"exact-generation terminal-state verification",
		"unverified ownership",
		"every matching path/state generation",
		"not caller-carried",
		...
		"do not rely on an EOF or half-close",
		"`devtmpfs.mount=0`",
		"`--enable-pci`",
		`"is_read_only": true`,
		`"guest_cid": 3`,
		...
		"fixed ext4 UUID",
		"fixed `hal-l5-build` container hostname",
...
func TestL5VerificationLocksLiveSelectorAndBroadGates(t *testing.T) {
	...
	for _, required := range []string{
		...
		"fixed stat/cat allowlist",
		"exclusive per-runtime lifecycle reservation",
		"stop/delete cannot report success during an in-flight start",
		"host-process ownership begins at launch",
		"Missing terminal verification fails closed",
