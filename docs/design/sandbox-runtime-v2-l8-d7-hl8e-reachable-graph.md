...
PID1-only `socket`, `bind`, `listen`, `dup3`, `close`, `prctl`,
`seccomp`, then `exit_group(127)`. PID1 attempts that listen-table
subset, installs the launch-base filter, and still fail-closes;
unimplemented `execve`, `clone3`, and SCM_RIGHTS are not reachable
syscall sites. That is fail-closed live behavior, not HL8E
