...
matching safe result/event unions. `create_job` carries only revision, the
controller-minted job, monitor, mount, and cgroup generations, fixed limit-set
ID, and digests. PID1
creates the cgroup and monitor and returns a safe generation result; the new
monitor sends its controller endpoint and live namespace authority through PID1 with
the exact two-right readiness handoff below, after which credential bytes use
that direct link. The matching `createJobSHA256` definition includes all four
generations, including `mountGeneration`.
...
outstanding helper request, exactly one concurrently populated credential-aware
workload, 4096 launches over the activation lifetime, 256 helper-owned file
