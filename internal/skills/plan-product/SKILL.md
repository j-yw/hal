...
Establish foundational product documentation through an interactive conversation. Creates mission, roadmap, and tech stack files in `.hal/product/`.
...
Check if `.hal/product/` exists and contains any of these files:
- `mission.md`
- `roadmap.md`
- `tech-stack.md`
...
Create the `.hal/product/` directory if it doesn't exist.
...
- The `hal plan --product` command will read these files when planning features, so having them populated helps with context
