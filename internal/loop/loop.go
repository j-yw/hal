				...
				if story := prd.CurrentStory(); story != nil {
					...
					prompt += fmt.Sprintf(
						"\n\n## IMPORTANT — Iteration %d Feedback\n"+
							...
							"5. Commit all changes, including `%s` and `%s`, with message `feat: %s - %s`\n"+
							"6. Run `git status --short` and confirm the working tree is clean\n\n"+
							"If you believe the code already satisfies the criteria, you STILL must update the PRD file, append progress, and commit all changes.\n"+
						...
						r.config.ProgressFile, r.config.PRDFile, r.config.ProgressFile, story.ID, story.Title,
					)
