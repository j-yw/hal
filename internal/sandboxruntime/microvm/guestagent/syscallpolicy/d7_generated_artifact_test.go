...
func TestL8D7GeneratedArtifactImportsCanonicalNonzeroAuthority(t *testing.T) {
	...
	if len(artifact.Catalog()) < 300 || len(artifact.Rules()) != 10 || len(artifact.Transitions()) != 9 {
	...
	if len(artifact.Workload().Rules()) != 1 || len(artifact.Runtime().Rules()) != 1 || artifact.Runtime().GoVersion() != "go1.25.7" {
