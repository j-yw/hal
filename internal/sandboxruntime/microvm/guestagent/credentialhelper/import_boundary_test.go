...
func TestCredentialHelperContractImportGuardExcludesFutureImplementationFiles(t *testing.T) {
	...
	for _, name := range []string{"contracts.go", "registry.go", "opaque.go", "format.go", "core_contract_error.go", "core_capabilities.go", "core_requests.go", "core_results.go", "core_accessors.go", "transport_types.go", "transport_accessors.go", "transport_receive.go", "transport_send.go"} {
		if !isCredentialHelperContractFile(name) {
...
func isCredentialHelperContractFile(name string) bool {
	switch name {
	case "contracts.go", "registry.go", "opaque.go", "format.go", "core_contract_error.go", "core_capabilities.go", "core_requests.go", "core_results.go", "core_accessors.go", "transport_types.go", "transport_accessors.go", "transport_receive.go", "transport_send.go":
		return true
	default:
		return false
	}
