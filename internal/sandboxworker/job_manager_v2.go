...
func newJobManagerV2(options jobManagerV2Options) (*jobManagerV2, error) {
	if !validWorkerV2SafeID(options.WorkerID) || !validWorkerV2SafeID(options.DaemonGeneration) {
	...
	for _, state := range retained {
		if state.CredentialState != nil {
			closeJobManagerV2StateLock(stateLock)
			return nil, errors.New("worker v2 credential recovery authority is required")
		}
	}
	...
	manager := &jobManagerV2{
		...
		daemonGeneration: options.DaemonGeneration,
...
func (manager *jobManagerV2) acceptCredentialSeed(driverID, principalID string, request JobStartRequestV2, seed sandboxruntime.JobCredentialIdentitySeed) (JobV2, bool, error) {
	...
	if manager == nil || manager.store == nil || request.Validate() != nil || sandboxruntime.ValidateJobCredentialIdentitySeed(seed) != nil ||
		...
		request.AdmissionGrantRevision != seed.AdmissionGrantRevision || strings.TrimSpace(request.TemplatePolicyID) != seed.TemplatePolicyID || strings.TrimSpace(request.WorkspacePolicyID) != seed.WorkspacePolicyID {
	...
	job := JobV2{
		...
		SubmittedAt:      seed.IssuedAt.UTC(),
		CredentialIntent: request.credentialIntent(),
	}
	state := storedJobStateV2{
		JobV2:            job,
		RequestKey:       requestKey,
		PrincipalID:      principalID,
		DaemonGeneration: manager.daemonGeneration,
