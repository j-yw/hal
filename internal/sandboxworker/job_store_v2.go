...
func (state storedJobStateV2) Validate() error {
	...
	if err := validateStoredJobCredentialStateV2(state); err != nil {
		return err
	}
	return nil
}
...
func validateStoredJobCredentialStateV2(state storedJobStateV2) error {
	...
	if !state.JobV2.CredentialIntent.ProductionCredentialsRequested || state.CredentialState.ContractVersion != storedJobCredentialContractVersionV2 ||
		state.CredentialState.Identity == nil && state.CredentialState.Revision != 0 {
		return errors.New("stored worker job credential state is invalid")
	}
	seed, err := state.CredentialState.Seed.runtimeSeed()
	if err != nil ||
		seed.WorkerID != state.JobV2.WorkerID || seed.WorkerJobID != state.JobV2.ID || seed.HostID != state.JobV2.HostID ||
		...
		seed.PrincipalID != state.PrincipalID || seed.TemplatePolicyID != state.JobV2.CredentialIntent.TemplatePolicyID || seed.WorkspacePolicyID != state.JobV2.CredentialIntent.WorkspacePolicyID ||
		!storedJobCredentialBindingsExactV2(seed, state.JobV2.CredentialIntent) {
...
func newStoredJobCredentialStateV2(seed sandboxruntime.JobCredentialIdentitySeed) (*storedJobCredentialStateV2, error) {
	...
	return &storedJobCredentialStateV2{ContractVersion: storedJobCredentialContractVersionV2, Seed: storedJobCredentialSeedV1(cloned)}, nil
}
