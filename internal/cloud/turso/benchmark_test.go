...
func TestBenchmarkClaimContention(t *testing.T) {
	...
	result := storetest.BenchmarkClaimContention(newStore, "turso", cfg)

	if result.Scenario != "claim_contention" {
		t.Errorf("scenario = %q, want %q", result.Scenario, "claim_contention")
	}
	if result.Adapter != "turso" {
		t.Errorf("adapter = %q, want %q", result.Adapter, "turso")
	}
	...
	writeReportArtifact(t, "turso_claim_contention", data)
}
...
func TestBenchmarkAuthLockContention(t *testing.T) {
	...
	result := storetest.BenchmarkAuthLockContention(newStore, setupAuthProfile, "turso", cfg)
	...
	writeReportArtifact(t, "turso_auth_lock_contention", data)
}
