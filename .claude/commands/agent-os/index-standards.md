...
1. List all `.md` files in `.hal/standards/` and its subfolders
...
**Note:** `root` is a reserved keyword — it refers to `.md` files directly in `.hal/standards/` (not in a subfolder). Do not create an actual folder named "root".
...
Generate `.hal/standards/index.yml` with this structure:
...
**Note:** `root` appears first and contains standards files that live directly in `.hal/standards/` (not in subfolders).
...
Updates `.hal/standards/index.yml`
