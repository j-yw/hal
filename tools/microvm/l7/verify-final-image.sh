...
for path in /sbin/init /sbin/hal-init /usr/bin/hal-guest-agent /bin/busybox /usr/bin/setpriv; do
...
grep -Fq 'agent:x:1000:1000:Agent:/workspace:/bin/sh' "$scratch/passwd"
debugfs -R 'cat /etc/group' "$image" >"$scratch/group" 2>/dev/null
grep -Fq 'agent:x:1000:' "$scratch/group"
