...
scratch=$(mktemp -d --tmpdir="$output_parent" .hal-l8-verify.XXXXXXXXXX)
...
for artifact in SHA256SUMS distribution-manifest.json final-inspection.json provenance.json rootfs.ext4 sources.lock.json vmlinux; do
...
if grep -aRF -- "$build_a" "$build_a" "$build_b" >/dev/null ||
	...
	echo "caller output path leaked into an L8 artifact" >&2
	exit 1
fi
if [[ ! -e "$output" ]]; then
	mkdir -m 0700 -- "$output"
